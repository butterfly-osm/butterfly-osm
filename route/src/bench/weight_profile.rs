...
pub fn run_weight_profile(data_dir: &Path, output_dir: &Path, region: Option<&str>) -> Result<()> {
    ...
    for (i, name) in state.mode_names.iter().enumerate() {
        let mode_data = state.get_mode(butterfly_route::model::types::Mode(i as u8));
        println!(
    ...
    for (mode_idx, mode_name) in state.mode_names.iter().enumerate() {
        let mode_data = state.get_mode(butterfly_route::model::types::Mode(mode_idx as u8));
...
fn snap_od_pairs(state: &ServerState, pairs: &[OdPair], mode_idx: u8) -> Vec<(u32, u32)> {
    let mode_data = state.get_mode(butterfly_route::model::types::Mode(mode_idx));
    pairs
        .iter()
        .filter_map(|p| {
            let src_orig = state
