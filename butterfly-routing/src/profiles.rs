...
impl HighwayType {
    ...
    pub fn default_access(&self, profile: TransportProfile) -> AccessLevel {
        match (self, profile) {
            ...
            (
                Self::Trunk | Self::TrunkLink | Self::Primary | Self::PrimaryLink,
...
impl AccessTruthTable {
    ...
    pub fn new() -> Self {
        ...
        for profile in TransportProfile::all() {
            table
                .profile_rules
                .insert(profile, Self::create_profile_rules(profile));
        }
    ...
    fn create_profile_rules(profile: TransportProfile) -> ProfileAccessRules {
        ...
        let highway_types = [
            ...
            HighwayType::Steps,
            HighwayType::Corridor,
            HighwayType::Platform,
            HighwayType::Ferry,
        ];
        ...
        match profile {
            TransportProfile::Car => {
                ...
                cases.push(SpecialAccessCase {
                    conditions: [("highway".to_string(), "footway".to_string())]
                        .into_iter()
                        .collect(),
                ...
                cases.push(SpecialAccessCase {
                    conditions: [("highway".to_string(), "cycleway".to_string())]
                        .into_iter()
                        .collect(),
                    access: AccessLevel::No,
                ...
                cases.push(SpecialAccessCase {
                    conditions: [("highway".to_string(), "steps".to_string())]
                        .into_iter()
                        .collect(),
                    access: AccessLevel::No,
            ...
            TransportProfile::Bicycle => {
                ...
                cases.push(SpecialAccessCase {
                    conditions: [("highway".to_string(), "motorway".to_string())]
                        .into_iter()
                        .collect(),
                    access: AccessLevel::No,
                ...
                cases.push(SpecialAccessCase {
                    conditions: [("highway".to_string(), "motorway_link".to_string())]
                        .into_iter()
                        .collect(),
                    access: AccessLevel::No,
    ...
    pub fn evaluate_all_profiles(
        &self,
        tags: &HashMap<String, String>,
    ) -> HashMap<TransportProfile, WayAccess> {
...
impl ProfileMask {
    ...
    pub fn apply_to_adjacency_list(
        &self,
        adjacency: &HashMap<i64, Vec<(i64, EdgeId)>>,
    ) -> HashMap<i64, Vec<(i64, EdgeId)>> {
...
impl MultiProfileMask {
    ...
    pub fn evaluate_way_for_all_profiles(
        &mut self,
        edge_id: EdgeId,
        tags: &HashMap<String, String>,
    ) -> HashMap<TransportProfile, bool> {
        let mut results = HashMap::new();
    ...
    pub fn validate_masks(
        &self,
        test_ways: &[(EdgeId, HashMap<String, String>)],
...
impl ComponentAnalyzer {
    ...
    pub fn analyze_components(&mut self, adjacency: &HashMap<i64, Vec<(i64, EdgeId)>>) {
        ...
        self.components
            .sort_by(|a, b| b.nodes.len().cmp(&a.nodes.len()));
    ...
    fn classify_and_preserve_components(&mut self) {
        ...
        for component in &mut self.components {
            ...
            component.is_preserved = match component.component_type {
                ...
                ComponentType::LegitimateIsland => {
                    Self::is_legitimate_island_static(component, self.profile)
                }
    ...
    pub fn prune_adjacency_list(
        ...
        for (node_id, neighbors) in adjacency {
            if self.should_preserve_node(*node_id) {
                let preserved_neighbors: Vec<_> = neighbors
                    .iter()
        ...
        let params = match profile {
            ...
            TransportProfile::Foot => {
                ...
                let k_up = if telemetry.delta_h90 > 1e-3f32 {
                    (target_time_increase * telemetry.delta_h90 / base_speed_mps)
                        / telemetry.delta_h90
                } else {
...
impl SpeedWeightCalculator {
    ...
    pub fn calculate_edge_weight(
        &self,
        way_access: &WayAccess,
        distance_meters: f32,
        tags: &HashMap<String, String>,
        grade_percent: Option<f32>,
    ) -> EdgeWeight {
        // Get base speed from highway type
        let highway_config = self
            .highway_speeds
            .get(&way_access.highway)
            .cloned()
            .unwrap_or(SpeedConfig {
                default_speed: 20.0,
                max_speed: 30.0,
                min_speed: 5.0,
            });
        ...
        if let Some(maxspeed_str) = tags.get("maxspeed") {
            if let Ok(maxspeed) = Self::parse_speed(maxspeed_str) {
                base_speed = maxspeed
                    .min(highway_config.max_speed)
                    .max(highway_config.min_speed);
            }
        }
        ...
        let grade_factor = if let Some(grade) = grade_percent {
            ...
            let modulated_penalties = self
                .grade_penalties
                .get_surface_modulated_params(surface_factor);
    ...
    fn parse_speed(speed_str: &str) -> Result<f32, String> {
        ...
        let numeric_part: String = speed_str
            .chars()
...
impl MultiProfileLoader {
        ...
        for profile in TransportProfile::all() {
            ...
            for (way_id, _nodes, tags) in ways {
                ...
                if way_access.should_include_in_graph() {
                    ...
                    let edge_weight =
                        speed_calc.calculate_edge_weight(&way_access, 100.0, tags, None);
    ...
    pub fn handle_route_echo(
        ...
        let speed_calc = &self.speed_calculators[&profile];
        let avg_speed = if let Some(residential_config) = speed_calc
            .get_highway_speeds()
            .get(&HighwayType::Residential)
        {
        ...
        RouteEchoResponse {
            ...
            coordinates,
            echo: format!(
                "Route echo for {} profile - no actual routing implemented yet",
                profile_name
            ),
...
impl SyntheticJunctionTests {
        ...
        for highway in &highway_types {
            for profile in TransportProfile::all() {
                tests.push(JunctionTestCase {
                    ...
                    tags: [("highway".to_string(), highway.to_string())]
                        .into_iter()
                        .collect(),
                    profile,
    ...
    fn generate_special_case_tests() -> Vec<JunctionTestCase> {
        ...
        for profile in TransportProfile::all() {
            tests.push(JunctionTestCase {
                ...
                tags: [("highway".to_string(), "bridleway".to_string())]
                    .into_iter()
                    .collect(),
                profile,
                expected_accessible: profile != TransportProfile::Car,
                expected_access_level: if profile == TransportProfile::Car {
                    AccessLevel::No
                } else {
                    AccessLevel::Yes
                },
    ...
    fn generate_edge_cases() -> Vec<JunctionTestCase> {
        ...
        for profile in TransportProfile::all() {
            tests.push(JunctionTestCase {
                ...
                tags: [("highway".to_string(), "unknown_type".to_string())]
                    .into_iter()
                    .collect(),
                profile,
        ...
        for profile in TransportProfile::all() {
            tests.push(JunctionTestCase {
                ...
                tags: [("highway".to_string(), "ferry".to_string())]
                    .into_iter()
                    .collect(),
                profile,
                expected_accessible: true,
    ...
    fn expected_highway_access(highway: &str, profile: TransportProfile) -> bool {
        HighwayType::parse(highway)
            .default_access(profile)
            .allows_routing(profile)
    }
...
mod tests {
    ...
    fn test_highway_default_access() {
        // Motorways - cars only
        assert_eq!(
            HighwayType::Motorway.default_access(TransportProfile::Car),
            AccessLevel::Yes
        );
        assert_eq!(
            HighwayType::Motorway.default_access(TransportProfile::Bicycle),
            AccessLevel::No
        );
        assert_eq!(
            HighwayType::Motorway.default_access(TransportProfile::Foot),
            AccessLevel::No
        );

        // Residential - all modes allowed
        assert_eq!(
            HighwayType::Residential.default_access(TransportProfile::Car),
            AccessLevel::Yes
        );
        assert_eq!(
            HighwayType::Residential.default_access(TransportProfile::Bicycle),
            AccessLevel::Yes
        );
        assert_eq!(
            HighwayType::Residential.default_access(TransportProfile::Foot),
            AccessLevel::Yes
        );

        // Footways - foot only (bikes generally not allowed)
        assert_eq!(
            HighwayType::Footway.default_access(TransportProfile::Car),
            AccessLevel::No
        );
        assert_eq!(
            HighwayType::Footway.default_access(TransportProfile::Bicycle),
            AccessLevel::No
        );
        assert_eq!(
            HighwayType::Footway.default_access(TransportProfile::Foot),
            AccessLevel::Yes
        );

        // Steps - foot only
        assert_eq!(
            HighwayType::Steps.default_access(TransportProfile::Car),
            AccessLevel::No
        );
        assert_eq!(
            HighwayType::Steps.default_access(TransportProfile::Bicycle),
            AccessLevel::No
        );
        assert_eq!(
            HighwayType::Steps.default_access(TransportProfile::Foot),
            AccessLevel::Yes
        );
    }
    ...
    fn test_access_truth_table_tag_overrides() {
        ...
        let footway_bike_tags = create_tags(&[("highway", "footway"), ("bicycle", "designated")]);
        ...
        let cycleway_foot_tags = create_tags(&[("highway", "cycleway"), ("foot", "designated")]);
    ...
    fn test_access_truth_table_private_access() {
        ...
        let private_tags = create_tags(&[("highway", "residential"), ("access", "private")]);
    ...
    fn test_access_truth_table_tag_priority() {
        ...
        let specific_tags =
            create_tags(&[("highway", "residential"), ("access", "no"), ("car", "yes")]);
    ...
    fn test_way_access_penalties() {
        ...
        for (access_value, expected_penalty) in test_cases {
            let tags = create_tags(&[("highway", "residential"), ("access", access_value)]);

            let access = table.evaluate_way_access(TransportProfile::Car, &tags);
            assert!(
                (access.penalty - expected_penalty).abs() < 0.01,
    ...
    fn test_synthetic_junction_tests_count() {
        ...
        assert!(
            tests.len() >= 100,
            "M4.1 requires 100+ synthetic tests, got {}",
            tests.len()
        );
        ...
        for profile in TransportProfile::all() {
            assert!(
                profile_counts.get(&profile).unwrap_or(&0) > &10,
                "Profile {:?} should have >10 tests, got {}",
                profile,
                profile_counts.get(&profile).unwrap_or(&0)
            );
        }
    ...
    fn test_run_all_synthetic_junction_tests() {
        ...
        println!(
            "Synthetic test results: {}/{} passed ({:.1}%)",
            results.passed,
            results.total_tests,
            results.pass_rate()
        );
        ...
        if !results.failures.is_empty() {
            ...
            for (i, failure) in results.failures.iter().take(5).enumerate() {
                println!(
                    "  {}. {}: expected accessible={}, got accessible={}",
                    i + 1,
                    failure.test_name,
                    failure.expected_accessible,
                    failure.actual_accessible
                );
                println!(
                    "     Expected access level: {:?}, got: {:?}",
        ...
        assert!(
            results.all_passed(),
            "Synthetic junction tests failed: {}/{} passed",
            results.passed,
            results.total_tests
        );
    }
    ...
    fn test_specific_synthetic_scenarios() {
        ...
        let test_cases = vec![
            ...
            (
                create_tags(&[("highway", "footway")]),
                TransportProfile::Car,
                false,
            ),
            // Bikes should not access motorways
            (
                create_tags(&[("highway", "motorway")]),
                TransportProfile::Bicycle,
                false,
            ),
            ...
            (
                create_tags(&[("highway", "residential")]),
                TransportProfile::Car,
                true,
            ),
            (
        ...
        for (tags, profile, expected_accessible) in test_cases {
            ...
            assert_eq!(
                access.is_accessible, expected_accessible,
    ...
    fn test_profile_mask_validation() {
        ...
        let test_ways = vec![
            ...
            (
                EdgeId(5),
                create_tags(&[("highway", "service"), ("access", "private")]),
            ),
        ];
    ...
    fn test_component_analysis_profiles() {
        ...
        for profile in TransportProfile::all() {
            ...
            println!(
                "Profile {:?}: {} components, largest size: {}, preservation rate: {:.1}%",
                profile,
                stats.total_components,
                stats.largest_component_size,
                stats.preservation_rate()
            );
    ...
    fn test_component_preservation() {
        ...
        println!(
            "Found {} components: {} preserved, {} pruned",
            stats.total_components,
            preserved.len(),
            pruned.len()
        );
        ...
        assert!(
            preserved.len() >= 2,
            "Should preserve at least main and secondary components"
        );
        ...
        assert!(
            analyzer.should_preserve_node(1),
            "Node 1 (main component) should be preserved"
        );
        assert!(
            analyzer.should_preserve_node(50),
            "Node 50 (secondary component) should be preserved"
        );
        assert!(
            !analyzer.should_preserve_node(200),
            "Node 200 (isolated) should be pruned"
        );

        // Test edge preservation
        assert!(
            analyzer.should_preserve_edge(1, 2),
            "Edge 1-2 (main component) should be preserved"
        );
        assert!(
            !analyzer.should_preserve_edge(1, 200),
            "Edge 1-200 (to pruned node) should not be preserved"
        );
    }
    ...
    fn test_adjacency_list_pruning() {
        ...
        for i in 1..=30 {
            assert!(
                pruned_adjacency.contains_key(&i),
                "Main component node {} should be preserved",
                i
            );
        }
        ...
        assert!(
            !pruned_adjacency.contains_key(&100),
            "Small island node 100 should be pruned"
        );
        assert!(
            !pruned_adjacency.contains_key(&101),
    ...
    fn test_component_statistics() {
        ...
        for i in 300..=374 {
            adjacency.insert(
                i,
                vec![(if i == 374 { 300 } else { i + 1 }, EdgeId(i + 200))],
            );
        }
        ...
        for base in [500, 600] {
            for i in 0..5 {
                adjacency.insert(
                    base + i,
                    vec![(if i == 4 { base } else { base + i + 1 }, EdgeId(base + i))],
                );
            }
        }
        ...
        println!(
            "  Preserved: {} components, {} nodes ({:.1}%)",
            stats.preserved_components,
            stats.preserved_nodes,
            stats.preservation_rate()
        );
        println!(
            "  Pruned: {} components, {} nodes ({:.1}%)",
            stats.pruned_components,
            stats.pruned_nodes,
            stats.pruning_rate()
        );
        println!(
        ...
        println!(
            "  Second largest: {} nodes",
            stats.second_largest_component_size
        );
    ...
    fn test_speed_weight_calculator_basic() {
        ...
        assert!(
            (weight.time_seconds - 120.0).abs() < 1.0,
        ...
        let dequantized = SpeedWeightCalculator::dequantize_weight(weight.quantized_weight);
        assert!(
            (dequantized - weight.time_seconds).abs() < 0.1,
            "Quantization error too large: {:.3}s",
            (dequantized - weight.time_seconds).abs()
        );
    }
    ...
    fn test_highway_speed_tables() {
        ...
        let residential_car = car_calc
            .get_highway_speeds()
            .get(&HighwayType::Residential)
            .unwrap();
        let residential_bike = bike_calc
            .get_highway_speeds()
            .get(&HighwayType::Residential)
            .unwrap();
        let residential_foot = foot_calc
            .get_highway_speeds()
            .get(&HighwayType::Residential)
            .unwrap();
    ...
    fn test_surface_modifiers() {
        ...
        assert!(
            bike_calc.get_surface_modifiers()["gravel"]
                > car_calc.get_surface_modifiers()["gravel"]
        );
        assert!(
            foot_calc.get_surface_modifiers()["gravel"]
                > bike_calc.get_surface_modifiers()["gravel"]
        );
    ...
    fn test_adaptive_grade_penalties() {
        ...
        // Test 5% downhill
        let downhill_weight =
            bike_calc.calculate_edge_weight(&way_access, 1000.0, &tags, Some(-5.0));
        ...
        assert!(
            ...
            expected_5pct_factor,
            actual_factor
        );

        // Test monotonicity: steeper grades should be slower
        let steep_uphill_weight =
            bike_calc.calculate_edge_weight(&way_access, 1000.0, &tags, Some(10.0));
        assert!(
            steep_uphill_weight.time_seconds > uphill_weight.time_seconds,
            "Steeper grades should be slower: 5%={:.1}s, 10%={:.1}s",
            uphill_weight.time_seconds,
            steep_uphill_weight.time_seconds
        );

        // Downhill should be faster than flat (with reasonable bounds)
        assert!(
            downhill_weight.time_seconds < flat_weight.time_seconds,
            "Downhill should be faster: flat={:.1}s, downhill={:.1}s",
            flat_weight.time_seconds,
            downhill_weight.time_seconds
        );
        ...
        assert!(
            downhill_factor <= 1.25, // Max 25% boost
            "Downhill boost should be capped: factor={:.2}x",
            downhill_factor
        );
    ...
    fn test_weight_penalties_combination() {
        ...
        assert!(
            slowdown_factor > expected_minimum,
        ...
        assert!(
            weight.penalties.surface_factor < 1.0,
            "Gravel should reduce speed"
        );
        assert!(
            weight.penalties.grade_factor > 1.0,
            "Uphill should increase time"
        );
        assert!(
            weight.penalties.access_factor > 1.0,
            "Destination access should add penalty"
        );
    }
    ...
    fn test_multi_profile_loader_access_tables() {
        ...
        assert!(
            car_table
                .evaluate_way_access(TransportProfile::Car, &motorway_tags)
                .is_accessible
        );
        assert!(
            !car_table
                .evaluate_way_access(TransportProfile::Car, &footway_tags)
                .is_accessible
        );
        assert!(
            !car_table
                .evaluate_way_access(TransportProfile::Car, &cycleway_tags)
                .is_accessible
        );
        ...
        assert!(
            !bike_table
                .evaluate_way_access(TransportProfile::Bicycle, &motorway_tags)
                .is_accessible
        );
        assert!(
            bike_table
                .evaluate_way_access(TransportProfile::Bicycle, &cycleway_tags)
                .is_accessible
        );
        ...
        assert!(
            !foot_table
                .evaluate_way_access(TransportProfile::Foot, &motorway_tags)
                .is_accessible
        );
        assert!(
            foot_table
                .evaluate_way_access(TransportProfile::Foot, &footway_tags)
                .is_accessible
        );
        assert!(
            !foot_table
                .evaluate_way_access(TransportProfile::Foot, &cycleway_tags)
                .is_accessible
        ); // Default no access
    }
    ...
    fn test_multi_profile_speed_differences() {
        ...
        let bike_response =
            loader.handle_route_echo(TransportProfile::Bicycle, coordinates.clone());
        let foot_response = loader.handle_route_echo(TransportProfile::Foot, coordinates.clone());

        // Car should be fastest
        assert!(
            car_response.profile_stats.avg_speed_kmh > bike_response.profile_stats.avg_speed_kmh
        );
        assert!(
            car_response.profile_stats.avg_speed_kmh > foot_response.profile_stats.avg_speed_kmh
        );

        // Bike should be faster than foot
        assert!(
            bike_response.profile_stats.avg_speed_kmh > foot_response.profile_stats.avg_speed_kmh
        );
    }
...
impl ProfileRegressionSuite {
    ...
    pub fn run_complete_test_suite(
        &mut self,
        ways_data: &[(i64, Vec<i64>, HashMap<String, String>)],
    ) -> PRSReport {
        let start_time = std::time::Instant::now();
    ...
    fn run_access_legality_tests(&mut self) {
        ...
        for test_case in test_cases {
            ...
            self.test_results
                .insert(format!("access_legality_{}", test_case.name), result);
        }
    }
    ...
    fn run_access_legality_test(&self, test_case: &AccessLegalityTestCase) -> TestResult {
        ...
        let message = if test_passed {
            format!(
                "Access legality correct for {} on {}",
                test_case.profile.name(),
                test_case.highway.name()
            )
        } else {
            format!(
                "Access legality failed: expected {}, got {} for {} on {}",
    ...
    fn run_routing_smoke_tests(&mut self) {
        ...
        for test_case in test_cases {
            ...
            self.test_results
                .insert(format!("routing_smoke_{}", test_case.name), result);
        }
    }
    ...
    fn run_routing_smoke_test(&self, test_case: &RoutingSmokeTestCase) -> TestResult {
        ...
        let message = if test_passed {
            format!(
                "Routing smoke test passed for {} profile",
                test_case.profile.name()
            )
        } else {
    ...
    /// Run forbidden edge analysis
    fn run_forbidden_edge_analysis(
        &mut self,
        ways_data: &[(i64, Vec<i64>, HashMap<String, String>)],
    ) {
        for profile in TransportProfile::all() {
            ...
            let test_result = TestResult {
                ...
                status: if forbidden_edges.is_empty() {
                    TestStatus::Pass
                } else {
                    TestStatus::Warning
                },
                profile,
                message: format!(
                    "Found {} potentially forbidden edges for {} profile",
                    forbidden_edges.len(),
                    profile.name()
                ),
            ...
            self.test_results
                .insert(test_result.test_name.clone(), test_result);
        }
    }

    /// Find forbidden edges for a specific profile
    fn find_forbidden_edges(
        &self,
        profile: TransportProfile,
        ways_data: &[(i64, Vec<i64>, HashMap<String, String>)],
    ) -> Vec<ForbiddenEdgeReport> {
            ...
            if let Some(highway_str) = tags.get("highway") {
                ...
                if expected_access == AccessLevel::Yes && !way_access.is_accessible {
                    forbidden_edges.push(ForbiddenEdgeReport {
                        ...
                        tags: tags.clone(),
                        reason: format!(
                            "Default access is Yes but evaluation blocked access due to: {}",
                            if tags.contains_key("access") {
                                "access tag restriction"
                            } else if tags.contains_key(&profile.name().to_lowercase()) {
                                "profile-specific tag restriction"
                            } else {
                                "unknown restriction"
                            }
                        ),
...
mod prs_tests {
    ...
    fn test_prs_v1_complete_test_suite() {
        ...
        let ways_data = vec![
            ...
            (
                5,
                vec![5, 6],
                create_tags(&[("highway", "residential"), ("access", "private")]),
            ),
        ];
    ...
    fn test_prs_access_legality_tests() {
        ...
        let ways_data = vec![(1, vec![1, 2], create_tags(&[("highway", "motorway")]))];

        let report = prs.run_complete_test_suite(&ways_data);

        // Filter access legality test results
        let access_tests: Vec<_> = report
            .test_results
            .iter()
        ...
        let car_tests: Vec<_> = access_tests
            .iter()
            .filter(|r| r.profile == TransportProfile::Car)
            .collect();
        let bike_tests: Vec<_> = access_tests
            .iter()
            .filter(|r| r.profile == TransportProfile::Bicycle)
            .collect();
        let foot_tests: Vec<_> = access_tests
            .iter()
    ...
    fn test_prs_routing_smoke_tests() {
        ...
        let ways_data = vec![(1, vec![1, 2], create_tags(&[("highway", "residential")]))];

        let report = prs.run_complete_test_suite(&ways_data);

        // Filter routing smoke test results
        let smoke_tests: Vec<_> = report
            .test_results
            .iter()
    ...
    fn test_prs_forbidden_edge_analysis() {
        ...
        let ways_data = vec![
            ...
            (
                2,
                vec![2, 3],
                create_tags(&[("highway", "primary"), ("car", "no")]),
            ),
            (
                3,
                vec![3, 4],
                create_tags(&[("highway", "footway"), ("foot", "no")]),
            ),
        ];
        ...
        let forbidden_tests: Vec<_> = report
            .test_results
            .iter()
