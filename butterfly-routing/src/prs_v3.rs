...
impl ProfileRegressionSuiteV3 {
    ...
    pub fn with_weight_system(mut self, weight_system: WeightCompressionSystem) -> Self {
        self.time_router = self.time_router.with_weight_compression(weight_system.clone());
    ...
    fn test_eta_plausibility(&mut self, profile: TransportProfile, test_routes: &[(NodeId, NodeId)]) -> PRSv3TestResult {
        let start_time = Instant::now();
        ...
        for &(start, end) in test_routes {
            ...
            if response.route_found {
                ...
                let expected_time = response.total_distance_meters / 1000.0 / 
                    ((speed_range.0 + speed_range.1) / 2.0) * 3600.0;
        ...
        PRSv3TestResult {
            ...
            message: format!(
                ...
                mean_error, implausible_rate * 100.0
            ),
            metrics,
            timestamp: std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
    ...
    fn test_turn_legality(&mut self, profile: TransportProfile, test_routes: &[(NodeId, NodeId)]) -> PRSv3TestResult {
        let start_time = Instant::now();
        let mut metrics = PRSv3Metrics::new();
    ...
    fn test_time_parity(&mut self, profile: TransportProfile, test_routes: &[(NodeId, NodeId)]) -> PRSv3TestResult {
        let start_time = Instant::now();
        let mut metrics = PRSv3Metrics::new();
    ...
    fn test_route_quality(&mut self, profile: TransportProfile, test_routes: &[(NodeId, NodeId)]) -> PRSv3TestResult {
        let start_time = Instant::now();
        let mut metrics = PRSv3Metrics::new();
        ...
        let poor_rate = *quality_counts.get(&RouteQuality::Poor).unwrap_or(&0) as f64 / test_routes.len() as f64;
        let not_found_rate = *quality_counts.get(&RouteQuality::NotFound).unwrap_or(&0) as f64 / test_routes.len() as f64;
        ...
        PRSv3TestResult {
            ...
            message: format!(
                ...
                poor_rate * 100.0, not_found_rate * 100.0
            ),
            metrics,
            timestamp: std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
    ...
    fn test_compression_accuracy(&mut self, profile: TransportProfile, test_routes: &[(NodeId, NodeId)]) -> PRSv3TestResult {
        let start_time = Instant::now();
        let mut metrics = PRSv3Metrics::new();
    ...
    fn test_shard_performance(&mut self, profile: TransportProfile, test_routes: &[(NodeId, NodeId)]) -> PRSv3TestResult {
        let start_time = Instant::now();
        let mut metrics = PRSv3Metrics::new();
    ...
    fn test_time_consistency(&mut self, profile: TransportProfile, test_routes: &[(NodeId, NodeId)]) -> PRSv3TestResult {
        let start_time = Instant::now();
        let mut metrics = PRSv3Metrics::new();
    ...
    fn test_profile_differences(&mut self, profile: TransportProfile, test_routes: &[(NodeId, NodeId)]) -> PRSv3TestResult {
        let start_time = Instant::now();
        let mut metrics = PRSv3Metrics::new();

        // Compare with other profiles on a subset of routes
        let test_subset = &test_routes[..test_routes.len().min(10)];
        let other_profiles = vec![TransportProfile::Car, TransportProfile::Bicycle, TransportProfile::Foot]
            .into_iter()
            .filter(|&p| p != profile)
            .collect::<Vec<_>>();
            ...
            if base_response.route_found {
                for &other_profile in &other_profiles {
                    ...
                    if other_response.route_found {
                        ...
                        let time_diff = (base_response.total_time_seconds - other_response.total_time_seconds).abs();
    ...
    fn test_constraint_respect(&mut self, profile: TransportProfile, test_routes: &[(NodeId, NodeId)]) -> PRSv3TestResult {
        let start_time = Instant::now();
        let mut metrics = PRSv3Metrics::new();
