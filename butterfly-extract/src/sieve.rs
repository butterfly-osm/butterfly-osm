...
impl TagTruthTable {
    ...
    pub fn is_routing_relevant(&self, tags: &HashMap<String, String>) -> bool {
        ...
        if tags
            .keys()
            .any(|k| self.speed_tags.contains(k) || self.surface_tags.contains(k))
        {
            return true;
        }
...
mod tests {
    ...
    fn test_highway_classification() {
        ...
        assert_eq!(
            table.classify_highway("residential"),
        ...
        assert_eq!(
            table.classify_highway("unknown"),
            HighwayClass::Other("unknown".to_string())
        );
    }
    ...
    fn test_tag_sieve_relation_filtering() {
        ...
        let route_tags =
            create_test_tags(&[("type", "route"), ("route", "bus"), ("name", "Bus Line 42")]);
