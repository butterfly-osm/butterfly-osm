...
impl TelemetryCalculator {
    ...
    fn process_way(&mut self, nodes: &[i64], _highway_type: &str) {
        ...
        for i in 1..nodes.len() {
            ...
            if let (Some(&(lat1, lon1)), Some(&(lat2, lon2))) = 
                (self.node_coords.get(&node1_id), self.node_coords.get(&node2_id)) {
                ...
                if i > 1 {
                    ...
                    if let Some(&(lat0, lon0)) = self.node_coords.get(&node0_id) {
                        let angle_change = calculate_angle_change(
                ...
                for tile_id in tiles_for_segment {
                    ...
                    tile_metrics.add_way(
                        segment_length, 
                        if total_length > 0.0 { total_angle_change / total_length } else { 0.0 }
                    );
                }
            }
        }
        ...
        for (node_id, connections) in &self.node_connections {
            if connections.len() > 2 {
                if let Some(&(lat, lon)) = self.node_coords.get(node_id) {
                    ...
                    self.tiles.entry(tile_id).or_default().add_junction(complexity);
                }
            }
        }
    }
    ...
    pub fn generate_telemetry(&self) -> Vec<TileTelemetry> {
        self.tiles.iter().map(|(tile_id, metrics)| {
    ...
    pub fn filter_by_bbox(&self, min_lat: f64, max_lat: f64, min_lon: f64, max_lon: f64) -> Vec<TileTelemetry> {
        self.tiles.iter()
            .filter_map(|(tile_id, metrics)| {
                let bbox = tile_id.bbox();
                
                // Check if tile intersects with query bbox
                if bbox.max_lat >= min_lat && bbox.min_lat <= max_lat &&
                   bbox.max_lon >= min_lon && bbox.min_lon <= max_lon {
    ...
    pub fn global_percentiles(&self) -> GlobalPercentiles {
        let all_junction_counts: Vec<f64> = self.tiles.values()
            .map(|m| m.junction_count as f64).collect();
        let all_lengths: Vec<f64> = self.tiles.values()
            .map(|m| m.total_length_m).collect();
        let all_densities: Vec<f64> = self.tiles.values()
            .map(|m| m.total_length_m / (TILE_SIZE_METERS * TILE_SIZE_METERS)).collect();
