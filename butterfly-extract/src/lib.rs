...
pub use coarsening::{
    BendWindow, BorderEdge, BorderReconciliation, BoundarySide, CanonicalAdjacency, CanonicalStats,
    CoarseningPolicy, CollapsePolicy, CurvatureAnalyzer, EdgeInfo, GraphDebugger, GraphEdgeStats,
    GraphNodeStats, LocalAngle, NodeCanonicalizer, NodeMapper, NodeMappingStats, PolicySmoother,
    SegmentGuard, SegmentGuardReason, SemanticBreakpoints, SemanticImportance, SuperEdge,
    SuperEdgeConstructor, SuperEdgeStats, TileBoundary,
};
pub use pbf::{MemberType, OsmPrimitive, PbfError, PbfReader, RelationMember};
pub use sieve::{
    AccessLevel, FilteredNode, FilteredRelation, FilteredWay, HighwayClass, TagSieve,
    TagTruthTable, VehicleProfile,
};
pub use telemetry::{
    DensityClass, GlobalPercentiles, TelemetryCalculator, TileId, TileMetrics, TilePercentiles,
    TileTelemetry, TILE_SIZE_METERS,
};
...
impl Extractor {
    ...
    pub fn get_telemetry_for_bbox(
        ...
        self.telemetry
            .filter_by_bbox(min_lat, max_lat, min_lon, max_lon)
    }
    ...
    pub fn probe_canonical_mapping(
        &self,
        lat: f64,
        lon: f64,
        radius: f64,
        _node_id: Option<i64>,
    ) -> Vec<CanonicalNodeProbe> {
    ...
    pub fn generate_m3_artifacts<P: AsRef<Path>>(&mut self, output_dir: P) -> std::io::Result<()> {
        ...
        self.graph_debugger
            .analyze_adjacency(&self.canonical_adjacency);
        self.graph_debugger
            .analyze_super_edges(&self.super_edge_constructor);

        // Generate nodes.bin
        let nodes_bin = self
            .graph_debugger
            .generate_nodes_bin(&self.canonical_adjacency)?;
        fs::write(output_dir.join("nodes.bin"), nodes_bin)?;

        // Generate super_edges.bin
        let super_edges_bin = self
            .graph_debugger
            .generate_super_edges_bin(&self.super_edge_constructor)?;
        fs::write(output_dir.join("super_edges.bin"), super_edges_bin)?;

        // Generate geom.temp
        let geom_temp = self
            .graph_debugger
            .generate_geom_temp(&self.super_edge_constructor)?;
    ...
    pub fn get_edge_details(&self, edge_id: &str) -> Option<serde_json::Value> {
        self.graph_debugger
            .get_edge_details(edge_id, &self.canonical_adjacency)
    }
