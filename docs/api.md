...
- `exclude=` and `avoid_polygons=` both recustomise the CCH weights; results cache per `(mode, mask)` / `(mode, polygon, mask)` (`BUTTERFLY_AVOID_CACHE_CAP` default 8, `BUTTERFLY_EXCLUDE_CACHE_CAP` default 3) and a hit costs ~22 ms. A COLD one is **seconds to about a minute** and the engine picks the cheaper of two shapes per mask (#606): measured on Belgium, a ~900 m avoid polygon takes 17.7 s and `exclude=motorway` 59.1 s, with no mask able to exceed ~70 s. Each cached weight set pins ~1.5 GB. The cold computation runs off the tokio worker (`avoid::off_runtime`, #539) so the rest of the server stays responsive, and identical cold requests single-flight instead of each recomputing. Note that the 120 s request timeout does NOT interrupt it — a synchronous handler cannot be pre-empted, measured: a 228 s cold request returned 200, not 408. Surfaced in `/health.avoid_cache` and `/health.exclude_cache`.
...
**A batch is Flight, a single query is REST (#624).** The action takes a
batch of origins; the single `lon`/`lat` form is the batch of one. Every
contour of every origin comes from ONE pass per origin (#625 measured the
double pass a one-threshold bulk forced), one rayon task per origin,
through the very pipeline `GET /isochrone` runs — the same bytes, gated.

Params:

```json
{ "origins": [[lon, lat], ...],  // up to 10 000 — OR the single form:
  "lon": f64, "lat": f64,
...
| Column | Arrow type | Notes |
|--------|------------|-------|
| `origin_idx` | u32 | index of the origin in `origins` (0 for the single form) (#624) |
