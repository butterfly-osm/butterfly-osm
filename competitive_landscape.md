...
11. **Multimodal transit built in** — full RAPTOR stack with multi-feed merging (GTFS + NeTEx-EPIP for STIB) in the same process as the road router, sharing one foot CCH. No separate OTP/MOTIS service required. Brussels→Antwerp multimodal query in 35 ms p50; 311 transit queries/sec sustained via the Flight `transit_bulk` action (historical `/transit/bulk` figure, same engine). Cross-operator stop bridges, same-station hierarchy, ULTRA transfer preprocessing all wired automatically.
