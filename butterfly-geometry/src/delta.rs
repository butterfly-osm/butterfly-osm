    ...
    pub fn encode(&self, geometry: &[Point2D]) -> Result<FullFidelityGeometry, String> {
        ...
        for &next_point in denoised.iter().skip(1) {
            ...
            if let (Some(dx_mm), Some(dy_mm)) =
                (self.coord_to_delta_mm(dx_m), self.coord_to_delta_mm(dy_m))
            {
        ...
        let total_length = denoised.windows(2).map(|w| w[0].distance_to(&w[1])).sum();

        // Calculate compression ratio
        let original_size = geometry.len() * std::mem::size_of::<Point2D>();
        let compressed_size =
            std::mem::size_of::<Point2D>() + delta_points.len() * std::mem::size_of::<DeltaPoint>();
...
impl DeltaEncoder {
    pub fn create_full_fidelity(
        &self,
        geometry: &[Point2D],
    ) -> Result<FullFidelityGeometry, String> {
