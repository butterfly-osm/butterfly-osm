...
use butterfly_extract::{DensityClass, SemanticBreakpoints, TelemetryCalculator};
...
impl SnapSkeleton {
    pub fn new(
        points: Vec<Point2D>,
        heading_samples: Vec<HeadingSample>,
        total_length: f64,
        spacing_used: f64,
    ) -> Self {
        Self {
...
impl ArcLengthResampler {
    ...
    fn is_urban_density(&self, geometry: &[Point2D]) -> bool {
        if let Some(ref telemetry_calc) = self.telemetry {
            ...
            let tile_id =
                butterfly_extract::telemetry::TileId::from_coords(mid_point.x, mid_point.y);
    ...
    fn extract_semantic_breakpoints(&self, geometry: &[Point2D]) -> HashSet<usize> {
        ...
        if let Some(ref _semantic) = self.semantic_breakpoints {
            ...
            for (i, window) in geometry.windows(3).enumerate() {
                ...
                if angle > 30.0 {
                    // Significant turn threshold - likely routing-critical
            ...
            for (i, window) in geometry.windows(2).enumerate() {
                ...
                if distance > 100.0 {
                    // Unusual gap - potential intersection
    ...
    fn resample_arc_length_with_breakpoints(
        ...
        for (segment_idx, window) in self.windows(2).enumerate() {
            ...
            while cumulative_distance - last_point_distance >= spacing {
                ...
                let segment_progress =
                    (last_point_distance - (cumulative_distance - segment_length)) / segment_length;
        ...
        if resampled.len() >= 3 {
            ...
            for i in 1..resampled.len() - 1 {
                ...
                let is_semantic = self.iter().enumerate().any(|(orig_idx, orig_point)| {
...
impl ArcLengthResampler {
    pub fn create_snap_skeleton(&self, geometry: &[Point2D]) -> Result<SnapSkeleton, String> {
        ...
        use crate::resample::ResampleArcLenExtended;
        let resampled_points = geometry.to_vec().resample_arc_length_with_breakpoints(
            spacing,
...
mod tests {
    ...
    fn test_angle_guarding() {
        ...
        let has_corner_point = resampled
            .iter()
            .any(|p| (p.x - 10.0).abs() < 1.0 && (p.y - 5.0).abs() < 1.0);

        assert!(
            has_corner_point,
            "Corner point should be preserved by angle guard"
        );
    }
    ...
    fn test_semantic_breakpoint_preservation() {
        ...
        let resampled = geometry
            .resample_arc_length_with_breakpoints(5.0, 15.0, &breakpoints)
            .unwrap();

        // Verify semantic breakpoints are preserved
        let has_intersection = resampled
            .iter()
            .any(|p| (p.x - 10.0).abs() < 0.1 && (p.y - 0.0).abs() < 0.1);
        let has_turn = resampled
            .iter()
            .any(|p| (p.x - 30.0).abs() < 0.1 && (p.y - 10.0).abs() < 0.1);

        assert!(
            has_intersection,
            "Intersection semantic breakpoint should be preserved"
        );
    ...
    fn test_enhanced_pass_a_with_m1_m2_integration() {
        use butterfly_extract::{SemanticBreakpoints, TelemetryCalculator};
        ...
        assert!(
            point_density > 10.0,
            "Should have good point density with enhanced Pass A"
        );
    }
    ...
    fn test_semantic_breakpoint_extraction() {
        ...
        let semantic = SemanticBreakpoints::new();
        let resampler =
            ArcLengthResampler::with_integrations(5.0, 25.0, 12.0, 40.0, None, Some(semantic));
        ...
        assert!(
            breakpoints.len() > 0,
            "Should detect semantic breakpoints in geometry with turns"
        );
    }
    ...
    fn test_angle_preservation_with_semantics() {
        ...
        let resampled = geometry
            .resample_arc_length_with_breakpoints(2.0, 60.0, &breakpoints)
            .unwrap();

        // Even with high angle threshold (60°), semantic point should be preserved
        let has_turn_point = resampled
            .iter()
            .any(|p| (p.x - 10.0).abs() < 0.5 && (p.y - 5.0).abs() < 0.5);

        assert!(
            has_turn_point,
            "Semantic breakpoint should override angle threshold"
        );
    }
}
