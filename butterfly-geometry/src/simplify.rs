...
impl NavigationGeometry {
    ...
    pub fn get_chunks(&self) -> Vec<Vec<Point2D>> {
        ...
        for anchor in &self.anchors[1..] {  // Skip first anchor
            // Find points up to this anchor
            let end_idx = self.simplified_points.iter()
                .position(|p| (p.x - anchor.point.x).abs() < 1e-6 && (p.y - anchor.point.y).abs() < 1e-6)
...
impl NavigationSimplifier {
    ...
    fn calculate_hausdorff_distances(&self, original: &[Point2D], simplified: &[Point2D]) -> (f64, f64) {
    ...
    pub fn simplify_for_navigation(&self, geometry: &[Point2D]) -> Result<NavigationGeometry, String> {
        if geometry.len() < 2 {
        ...
        let (hausdorff_median, hausdorff_p95) = self.calculate_hausdorff_distances(geometry, &simplified);
        
        // Auto-fallback if quality gates fail
        if hausdorff_median > self.hausdorff_median_target || hausdorff_p95 > self.hausdorff_p95_target {
...
mod tests {
    ...
    fn test_segment_based_rdp_processing() {
        ...
        assert_eq!(*nav_geom.simplified_points.last().unwrap(), *geometry.last().unwrap());
    }
    ...
    fn test_segment_creation_and_merging() {
        ...
        // Test merging
        let simplified_segments: Vec<Vec<Point2D>> = segments.iter()
    ...
    fn test_segment_vs_standard_rdp_comparison() {
        ...
        let standard_simplifier = NavigationSimplifier::with_segment_config(
            2.0, 15.0, 512.0, 2.0, 5.0, 50.0,
            false, // Disable segment-based RDP
        );
        
        // Segment-based simplifier
        let segment_simplifier = NavigationSimplifier::with_segment_config(
            2.0, 15.0, 512.0, 2.0, 5.0, 50.0,
            true,  // Enable segment-based RDP
        );
        
        let standard_result = standard_simplifier.simplify_for_navigation(&geometry).unwrap();
        let segment_result = segment_simplifier.simplify_for_navigation(&geometry).unwrap();
    ...
    fn test_multi_pass_segment_fallback() {
        ...
        let simplifier = NavigationSimplifier::with_segment_config(
            ...
            15.0, 512.0,
            1.0,  // Very strict quality targets
            2.0,
            25.0, // Small segment threshold
            true,
        );
        
        let nav_geom = simplifier.simplify_for_navigation(&challenging_geometry).unwrap();
