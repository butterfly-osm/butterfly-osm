...
use crate::resample::{Point2D, ArcLengthResampler, SnapSkeleton};
use crate::simplify::{NavigationSimplifier, NavigationGeometry};
...
impl GeometryPipelineResult {
    ...
    pub fn total_memory_usage(&self) -> usize {
        ...
        let pass_c_size = self.pass_c_result
            .as_ref()
...
impl GeometryPipeline {
    pub fn new(config: PipelineConfig) -> Self {
        ...
        let pass_c_encoder = DeltaEncoder::new(
            config.noise_threshold,
            config.max_delta_meters,
        );
    ...
    pub fn process_geometry(&self, geometry: &[Point2D]) -> Result<GeometryPipelineResult, String> {
        ...
        let pass_a_result = self.pass_a_resampler.create_snap_skeleton(geometry)
            .map_err(|e| format!("Pass A failed: {}", e))?;
        stats.pass_a_points = pass_a_result.points.len();

        // Pass B: Navigation simplification  
        let pass_b_result = self.pass_b_simplifier.simplify_for_navigation(&pass_a_result.points)
        ...
        let pass_c_result = if self.config.enable_pass_c {
            let result = self.pass_c_encoder.create_full_fidelity(&pass_b_result.simplified_points)
    ...
    pub fn process_streaming(&self, geometry_stream: &[&[Point2D]]) -> Result<Vec<GeometryPipelineResult>, String> {
        ...
        for (batch_idx, batch) in geometry_stream.iter().enumerate() {
            ...
            if estimated_memory_mb > self.config.memory_limit_mb as f64 {
                return Err(format!("Batch {} exceeds memory limit: {:.1}MB > {}MB", 
    ...
    fn process_with_fallback(&self, geometry: &[Point2D], mut stats: ProcessingStats) -> Result<GeometryPipelineResult, String> {
        ...
        let pass_b_result = self.pass_b_simplifier.simplify_for_navigation(&all_a_points)?;
        stats.pass_b_points = pass_b_result.simplified_points.len();
...
mod tests {
    ...
    fn test_batch_creation() {
        let geometry: Vec<Point2D> = (0..100)
