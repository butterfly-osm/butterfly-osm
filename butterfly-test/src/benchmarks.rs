...
use crate::generators::{ProblematicRegionGenerator, KnownProblematicRegions};
...
impl GeometryBenchmarks {
    ...
    pub fn benchmark_distance_calculations(runner: &mut BenchmarkRunner) {
        ...
        let normal_points = (0..1000).map(|_| (
    ...
    pub fn benchmark_simplification(runner: &mut BenchmarkRunner) {
        ...
        // Test on normal data
        let normal_linestring = (0..10000).map(|i| {
            (i as f64 * 0.01, (i as f64 * 0.01).sin())
        }).collect::<Vec<_>>();
...
fn web_mercator_to_wgs84(point: (f64, f64)) -> (f64, f64) {
    ...
    let lat = (2.0 * ((point.1 * 180.0 / 20037508.34 * std::f64::consts::PI / 180.0).exp().atan()) - std::f64::consts::PI / 2.0) * 180.0 / std::f64::consts::PI;
    (lon, lat)
}
