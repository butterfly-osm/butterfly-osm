...
use butterfly_dl::{DownloadOptions, Downloader};
use std::fs;
use std::io::Read;
...
impl TestCorpus {
    ...
    pub async fn ensure_monaco_data(&self) -> Result<PathBuf, Box<dyn std::error::Error>> {
        ...
        self.downloader
            .download_to_file("europe/monaco", monaco_path.to_str().unwrap(), &options)
            .await?;
    ...
    pub fn validate_pbf_file<P: AsRef<Path>>(
        &self,
        path: P,
    ) -> Result<PbfInfo, Box<dyn std::error::Error>> {
