...
impl<T: Ord> LoserTree<T> {
    ...
    #[inline]
    pub fn extract_min_and_replace_winner(
        &mut self,
        new_entry: Option<LoserTreeEntry<T>>,
    ) -> Option<LoserTreeEntry<T>> {
        let w = self.winner;
    ...
    #[inline]
    pub fn extract_min_and_replace(
        &mut self,
        run_index: usize,
        new_entry: Option<LoserTreeEntry<T>>,
    ) -> Option<LoserTreeEntry<T>> {
        debug_assert_eq!(
            run_index, self.winner,
            "extract_min_and_replace: run_index must equal current winner"
        );
    ...
    pub fn active_runs(&self) -> usize {
        self.keys[..self.k].iter().filter(|x| x.is_some()).count()
    }
}
...
mod tests {
    ...
    fn test_loser_tree_extract_and_replace() {
        ...
        let extracted = tree.extract_min_and_replace(
            1,
            Some(LoserTreeEntry {
                value: 4,
                run_index: 1,
