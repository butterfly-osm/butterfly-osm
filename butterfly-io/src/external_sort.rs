...
fn read_rss_kb() -> IoResult<u64> {
    ...
    if fields.len() < 2 {
        return Err(IoError::InvalidFormat("Invalid /proc/self/statm format".to_string()));
    }
    ...
    T: Clone + Ord + serde::Serialize + for<'de> serde::Deserialize<'de>,
{
    ...
    fn spill_to_disk(&mut self) -> IoResult<()> {
        ...
        // Create spill file
        let spill_path = self.temp_dir.join(format!("run_{:04}.bfly", self.run_counter));
        self.run_counter += 1;
    ...
    T: Clone + Ord + serde::Serialize + for<'de> serde::Deserialize<'de> + 'static,
{
    fn push(&mut self, item: T) -> IoResult<()> {
        ...
        if !self.admission_controller.admit_with_timeout(Duration::from_secs(1)) {
            return Err(IoError::InvalidFormat("Worker admission rate limit exceeded".to_string()));
        }
    ...
    T: Clone + Ord + serde::Serialize + for<'de> serde::Deserialize<'de>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(min_entry) = self.loser_tree.peek_min() {
            ...
            self.loser_tree.extract_min_and_replace(run_index, next_entry);
...
mod tests {
    ...
    fn test_external_sorter_basic() {
        ...
        let mut sorter = MemoryThrottledSorter::new(temp_dir.path(), 1024).expect("Failed to create sorter");
    ...
    fn test_external_sorter_with_spilling() {
        ...
        let mut sorter = MemoryThrottledSorter::new(temp_dir.path(), 1).expect("Failed to create sorter");
